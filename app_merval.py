...
with tab2:
    ...
    df_inf = pd.DataFrame({"Mes": meses_2026, "Inflación (%)": inf_proyectada})
    
    # Verificación del 21% anual
    total_anual = (np.prod([(1 + x/100) for x in inf_proyectada]) - 1) * 100
    
    c1, c2 = st.columns([1, 2])
    with c1:
        st.table(df_inf)
        st.metric("Acumulada Dic-26", f"{total_anual:.1f}%")
    with c2:
        fig_inf = go.Figure()
        ...
        fig_inf.update_layout(template="plotly_white", yaxis=dict(range=[0, 3]))
        st.plotly_chart(fig_inf, use_container_width=True)

# --- PESTAÑA 3: BONOS & COMPARATIVA ---
with tab3:
    st.header("💸 Bonos Tasa Fija vs Plazo Fijo")
    tasa_pf_nacion = 3.2 # TEM Mensual Plazo Fijo Nación
    ...
    st.subheader("⚖️ Comparativa")
    m1, m2 = st.columns(2)
    m1.metric("TEM Plazo Fijo BNA", f"{tasa_pf_nacion}%")
